...
| Item | Starting point |
|---|---|
| Tagged directory | https://github.com/angular/components/tree/16.2.14/src/material/legacy-paginator |
| Public API inventory anchor | https://raw.githubusercontent.com/angular/components/16.2.14/src/material/legacy-paginator/public-api.ts |
| Source closure to inspect | legacy form-field/select/core plus ordinary paginator intl/types |
| Current public candidates | Public MatPaginatorIntl when identity/shape match |
| Motion audit | select integration |
| Upstream recipe IDs | THEME-01 |

## Concrete preservation checks

labels/intl updates, page options, input coercion, buttons/keyboard, page event ordering.

**Do not take this shortcut:** Paginator is in scope. Preserve page-size selector markup and distinct default token behavior.
...
Store exact source/target SHAs, changed files, commands/results and evidence locations in `compatibility/work-items/legacy-paginator.json`. Proposed dependency candidates are not permission to claim runtime compatibility without tests.
