...
| Item | Starting point |
|---|---|
| Tagged directory | https://github.com/angular/components/tree/16.2.14/src/material/legacy-chips |
| Public API inventory anchor | https://raw.githubusercontent.com/angular/components/16.2.14/src/material/legacy-chips/public-api.ts |
| Source closure to inspect | legacy option-like selection and form-field contract; chip list/input/remove |
| Current public candidates | SelectionModel, key manager, FocusMonitor, Directionality |
| Motion audit | remove/selection and focus ordering |
| Upstream recipe IDs | ROB-03 |

## Concrete preservation checks

separator keys, add/remove, multi-selection, CVA/errors, focus after removal.

**Do not take this shortcut:** Preserve old chip-list API, not modern chip grid/listbox rewrites; capture nested content and removal lifecycle.
...
Store exact source/target SHAs, changed files, commands/results and evidence locations in `compatibility/work-items/legacy-chips.json`. Proposed dependency candidates are not permission to claim runtime compatibility without tests.
