...
| Item | Starting point |
|---|---|
| Tagged directory | https://github.com/angular/components/tree/16.2.14/src/material/legacy-progress-spinner |
| Public API inventory anchor | https://raw.githubusercontent.com/angular/components/16.2.14/src/material/legacy-progress-spinner/public-api.ts |
| Source closure to inspect | legacy spinner base and svg geometry |
| Current public candidates | Angular/platform/coercion as needed |
| Motion audit | CSS/SVG animation cycles |
| Upstream recipe IDs | THEME-01 |

## Concrete preservation checks

diameter, stroke width, mode, invalid values, ARIA, nested themes.

**Do not take this shortcut:** Compare viewBox, stroke/dash values and host/SVG classes, not only screenshot at one cycle time.
...
Store exact source/target SHAs, changed files, commands/results and evidence locations in `compatibility/work-items/legacy-progress-spinner.json`. Proposed dependency candidates are not permission to claim runtime compatibility without tests.
