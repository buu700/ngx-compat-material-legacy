...
| Item | Starting point |
|---|---|
| Tagged directory | https://github.com/angular/components/tree/16.2.14/src/material/legacy-card |
| Public API inventory anchor | https://raw.githubusercontent.com/angular/components/16.2.14/src/material/legacy-card/public-api.ts |
| Source closure to inspect | projected title/subtitle/header/content/actions/image directives |
| Current public candidates | Minimal Angular directives; no modern card inheritance |
| Motion audit | audit only; avoid inventing animations |
| Upstream recipe IDs | THEME-01 |

## Concrete preservation checks

all projection regions, image sizes, action alignment, appearance classes.

**Do not take this shortcut:** Preserve projected DOM and CSS margin/padding collapse; even simple cards need computed-style tests.
...
Store exact source/target SHAs, changed files, commands/results and evidence locations in `compatibility/work-items/legacy-card.json`. Proposed dependency candidates are not permission to claim runtime compatibility without tests.
